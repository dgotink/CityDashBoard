function createColorScheme(){
    var dict = {};
    dict['forground'] = {};
    dict['background'] = {};
    
    /*dict['background']['Antwerpen'] = '#CEC4E3';
    dict['background']['Brussel'] = '#E1F5D1';
    dict['background']['Leuven'] = '#E09B9B';
    dict['background']['temperature'] = '#FFDBBA';
    dict['background']['pressure'] = '#DDA0C3';
    dict['background']['humidity'] = '#BDD7DD';*/
    
    dict['background']['Antwerpen'] = '#FFFFFF';
    dict['background']['Brussel'] = '#FFFFFF';
    dict['background']['Leuven'] = '#FFFFFF';
    dict['background']['temperature'] = '#FFFFFF';
    dict['background']['pressure'] = '#FFFFFF';
    dict['background']['humidity'] = '#FFFFFF';
    
    dict['forground']['Antwerpen'] = '#F4A261';
    dict['forground']['Brussel'] = '#E9C46A';
    dict['forground']['Leuven'] = '#E76F51';
    dict['forground']['temperature'] = '#247BA0';
    dict['forground']['pressure'] = '#70C1B3';
    dict['forground']['humidity'] = '#B2DBBF';
    
    /*dict['background']['Antwerpen'] = '#688484';
    dict['background']['Brussel'] = '#BFCDA2';
    dict['background']['Leuven'] = '#DCAEAE';
    dict['background']['temperature'] = '#FFDBBA';
    dict['background']['pressure'] = '#DDA0C3';
    dict['background']['humidity'] = '#BDD7DD';
    
    dict['forground']['Antwerpen'] = '#335D5D';
    dict['forground']['Brussel'] = '#7B9150';
    dict['forground']['Leuven'] = '#9B5656';
    dict['forground']['temperature'] = '#B17B49';
    dict['forground']['pressure'] = '#A4457B';
    dict['forground']['humidity'] = '#598B97';*/
   
    return dict;
}